...
public class SystemMsgTypeSupport extends TypeSupport {
    private String type_name = "SystemMsg";
    private static TypeCodeImpl s_typeCode = null;
    private static SystemMsgTypeSupport m_instance = new SystemMsgTypeSupport();
    ...
    public int copy_sampleI(Object dst,Object src) {
        SystemMsg SystemMsgDst = (SystemMsg)dst;
        SystemMsg SystemMsgSrc = (SystemMsg)src;
        SystemMsgDst.copy(SystemMsgSrc);
        return 1;
    }
    ...
    public int print_sample(Object _sample) {
        ...
        SystemMsg sample = (SystemMsg)_sample;
        if (sample.msg_type != null){
            System.out.println("sample.msg_type:" + sample.msg_type);
        }
        else{
            System.out.println("sample.msg_type: null");
        }
    ...
    public DataReader create_data_reader() {return new SystemMsgDataReader();}

    public DataWriter create_data_writer() {return new SystemMsgDataWriter();}
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        SystemMsg sample = (SystemMsg)_sample;
        offset += CDRSerializer.get_string_size(sample.msg_type == null ? 0 : sample.msg_type.getBytes().length, offset);
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        if (!CDRSerializer.put_string(cdr, sample.msg_type, sample.msg_type == null ? 0 : sample.msg_type.length())){
            System.out.println("serialize sample.msg_type failed.");
            return -2;
        }
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        SystemMsg sample = (SystemMsg) _sample;
        sample.msg_type = CDRDeserializer.get_string(cdr);
        if(sample.msg_type ==null){
            System.out.println("deserialize member sample.msg_type failed.");
            return -3;
        }
    ...
    public TypeCode get_typecode(){
        ...
        s_typeCode = factory.create_struct_TC("SnakeGame.SystemMsg");
        if (s_typeCode == null){
            System.out.println("create struct SystemMsg typecode failed.");
            return s_typeCode;
        }
        int ret = 0;
        ...
        if (memberTc == null){
            System.out.println("Get Member msg_type TypeCode failed.");
